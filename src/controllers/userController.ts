...
export const uploadAvatar = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    cloudinaryResult = await uploadWithRetry(
      file,
      "tiktok_clone_avatars",
      "image",
    );
    ...
    if (
      ...
      currentUser.avatarUpdatedAt > requestTimestamp
    ) {
    ...
    return res.status(500).json(error("Lỗi upload avatar: " + message, 500));
  }
};
